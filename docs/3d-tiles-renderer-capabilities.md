...
Tellux 当前使用 `XYZTilesPlugin`，并固定 `shape: 'ellipsoid'`，用于把 template-url imagery provider 投到地球表面。
...
- 增加 WMS / WMTS imagery provider。
- 增加 Cesium Ion imagery overlay。
- 支持 GeoJSON / MVT / PMTiles 叠加。
- 支持多图层叠加和透明度控制。
