...
### Added
- 更新 globe.material 模式为自动
- 局部草地示例接入地球开关、物体级 RTC 与预设视角
- 将地表着色与透明度收到 viewer.globe
- 新增无球局部草地大气示例
- 将色调映射收口为 postProcess.toneMapping
- 移除旧的植被示例，更新 HISM 示例标题和描述
- 更新大气和云控件，添加新设置面板选项
- 更新高斯泼溅案例默认加载为 Cesium ion Redmond，并调整相机视角
- 添加水域外观和光学选项配置
- 将大气 world 到 ECEF 变换收口为 scene 门面
- 为大气增加 setWorldToECEFMatrix，支持局部 ENU 世界
- 体积云随相机高度淡出，避免高空硬切
- 将默认垂直 FOV 调为 50° 并增强相机飞行示例
- 将 Three.js 互操作示例切换为 WebGPU 并启用 TAA
- 新增模型局部光照与 Littlest Tokyo 夜景示例
- 接入跨渲染后端 Bloom 后处理
- 重构场景时钟并统一时间轴控制
- 抽取示例 GIS 数据源配置并支持本地天地图代理
- 完成剩余示例面板迁移到 Leva
- 更新社区展示案例，新增 Google 实景 3D Tiles 第三人称漫游条目
- 增加 Water Area 环境与场景反射
- 接入 WebGPU 镜头光晕后处理
- 接入 WebGPU 高精度速度与时间抗锯齿
- 完善 WebGPU 星空与后处理图基础设施
- 对齐 Water2 双相位水流
- 完善 WebGPU 水域外观渲染
- 新增 WebGPU 水域渲染案例
- 完善 Timeline 时间控制并收口 Clock
- 对齐场景配置同构并接入云外观与镜头光晕参数
- 新增社区案例 gallery 页与首页精选条

### Changed
- 移除构建体积预算相关功能和测试
- 统一可见性开关、数据源形状与实体描边模型
- 补齐运行时同构路径并收敛经纬高采样 API
- 统一 1.0 领域门面并将曝光收进 postProcess
- 落地 1.0 公开契约修正并收口内部实现
- 示例面板迁移至 Leva 并修复 Sandcastle 样式
- 移除 Water Area 场景反射
- 拆分 Sandcastle 专用能力并固化体积预算
- 将 HISM demo 收拢到 examples/hism
...
- 修复高斯案例数据源与底图纹理翻转
- 修复夜间瓦片受模型环境光影响
- 更新排除的 HTML 文件列表以包含 gallery.html
- 修复无法线点云显示色与光照语义
- 修复 Cesium Ion 点云无法解码和显示
- 收紧实体契约与场景状态边界
- 收紧高频拾取调度与空间筛选
- 收口 Viewer 与模型异步失败生命周期
- 收紧 peer 依赖构建边界
- 隔离 HISM BVH 拾取副作用
- 收口详细高度采样生命周期
- 补全 pickAll 全量拾取语义
