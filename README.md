...
- 使用大气天空、空气透视、体积云、昼夜光照、SMAA、镜头光晕和抖动等效果构建地理场景；WebGPU 可按需启用 TAA。
- 与 Three.js 场景、对象、坐标转换及自定义渲染循环互操作。
- 默认使用 WebGL；实验性的 WebGPU renderer 支持基础地球、地形、影像、3D Tiles、模型、拾取、大气与可选 TAA，具体限制见[能力边界](https://tellux.cyanfish.site/docs/guide/limitations)。
