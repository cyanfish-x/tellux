...
高斯案例出现底图错缝、或升级 Spark / Three.js 时，先读 [Spark 纹理上传状态缓存冲突](engineering/Spark纹理上传状态缓存冲突.md)，区分 URL 索引、上传状态缓存与 Vite 预构建；仅在相应路径变化时展开源码及回归测试。高斯被天空洗白时读 [无深度内容被大气天空分支覆盖](engineering/无深度内容被大气天空分支覆盖坑点.md)，不要先关大气或给 Spark 开 `depthWrite`。
