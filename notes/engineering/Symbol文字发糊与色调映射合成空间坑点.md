...
> 2026-09-06 复核范围：核对 Viewer 后合成调用与 MsdfAtlasLoader 的 mipmap 配置；收窄跨引擎与字体效果的绝对化表述，原画面观察未重跑。
...
> 与 [实体颜色被AgX色调映射压扁坑点](实体颜色被AgX色调映射压扁坑点.md) 的区别：实体不透明像素的 AgX 反求有效；半透明混合像素仍受非线性合成影响；symbol 的抗锯齿边缘是半透明的，反求在 coverage 渐变上失效。同一套反求补偿，对不透明实体可用，对半透明文字边缘不可用。

### 根因二：SMAA（默认开启）排在 symbol 之后

SMAA 是形态学抗锯齿，会检测小字形边缘并沿边模糊。原 13px 中文案例中 SMAA 会继续软化字形边缘。即使修好根因一，只要 symbol 还在 SMAA 之前的缓冲里，仍会被糊。

### 根因三：MSDF atlas 开了 mipmap

`MsdfAtlasLoader` 原先 `generateMipmaps=true` + `LinearMipmapLinearFilter`。MSDF 三通道存的是有符号距离，边角靠 `median(r,g,b)` 重建；mipmap 会把相邻纹素的距离盒式平均再跨层插值，median 重建失效、尖角抹圆、边缘发灰。**本项目 MSDF 距离纹理不使用普通平均 mipmap**，这是 MSDF 最典型的错误。atlas 字形 41px 渲染 13px 是 ~3:1 缩小，正好采样到被平均坏的 mip level。
...
1. **合成阶段**：symbol 是否在 tone mapping 之前画进 HDR 缓冲？是否在 SMAA/dithering 之前？若是，应先核对 coverage 是否经过非线性压缩。确认 `SymbolOcclusionPass.renderAfterComposite` 在 `rendererAdapter.render()` 之后被调用，且 `renderWithEffectsBypassed` 旁路了 effects 链。
2. **颜色空间**：symbol 颜色是否走了 `resolveDisplayColor` 而非 `resolveColor`？hex getter 是否用了 `getHex(LinearSRGBColorSpace)` 防二次编码？
3. **MSDF mipmap**：`MsdfAtlasLoader` 是否 `generateMipmaps=false` + 双 `LinearFilter`？（本项目不使用普通 MSDF mipmap）
...
- **symbol 永远在后合成阶段绘制**，不要为了"统一渲染"把它挪回 effects 链。本项目据此采用后合成方案——文字标签是 display-space overlay，不是 HDR 场景内容。
...
- **当前 MSDF 纹理保持 `generateMipmaps=false`**。普通平均 mipmap 不能无条件用于当前 MSDF 距离场——MSDF 的 AA 在 shader 里。
...
1. **合成阶段错了，这是"从根上错"的部分**。把 symbol 画进 HDR linear 缓冲再整帧 AgX，对文字抗锯齿是毁灭性的。本项目原路径会把 coverage 与 HDR 巨值混合再做非线性映射。其他引擎的具体合成链需独立核对，不作为本记录证明范围。
