...
3. 把 creased normals 做成全局 scene 开关。
   ...
   全局开关会影响基础地表、地形、普通 3D Tiles、采样专用 tileset 等多条路径，边界太宽。Takram 示例是在当前 Google tiles renderer 上挂插件，Tellux 中应收敛为 `viewer.tilesets.add` 图层级选项。
...
2. 把 `creasedNormals` 收敛为 3D Tiles 图层级选项。
   ...
   ```ts
   viewer.tilesets.add({
     source: { type: 'cesium-ion', assetId: 2275207, apiToken },
     creasedNormals: true
   })
   ```
