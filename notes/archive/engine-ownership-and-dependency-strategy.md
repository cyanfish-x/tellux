...
> 状态：**2026-07-02 设计背景，实施进度需按模块核对**。当前已有 `src/hism/` 与公开 `viewer.hism`，不再适用“整体待启动”的描述；原 chunk 计划及 vendor 建议不等于全部已采纳或实现。使用 HISM 见 [指南](../../docs/guide/hism.md)，当前装配见 [项目架构](../architecture/项目架构.md)。
...
2026-09-06 核对：PositionPipeline、RTC stage、自有风摆叶片材质与 HISM 已存在；外部自定义 shader 仍可能经过内联兼容路径，见 [当前 RTC 边界](../engineering/ez-tree风摆与RTC争抢project_vertex坑点.md)。下文估时、vendor 候选和 AI 协作建议均属原方案，不是当前任务授权或已确认性能。
...
1. **Shader 注入冲突**：vegetation 案例中 ez-tree 的风摆 shader 与 Tellux 自研 RTC（globe-scale 相对相机坐标）注入逻辑同时改写 Three.js `<project_vertex>` chunk，已存在两方字符串 patch 字符串的脆弱状态。任何第三层位置管线贡献者（GPU 实例剔除、impostor billboard、LOD 等）的加入都会使脆弱度成倍上升。详见 [ez-tree风摆与RTC争抢project_vertex坑点](../engineering/ez-tree风摆与RTC争抢project_vertex坑点.md)。
...
- 在 WebGPU 渲染模式下整个 `onBeforeCompile` 机制失效（见 [WebGPU下onBeforeCompile着色器机制失效坑点](../engineering/WebGPU下onBeforeCompile着色器机制失效坑点.md)）
...
**例外**：某个 plugin 已被全改时，可把该文件拷进 repo（原提案举例路径为 `src/tiles/vendored/FadePlugin.ts`，当前不存在，不是已落地文件），主库仍是依赖。
...
- 坑点：[ez-tree风摆与RTC争抢project_vertex坑点](../engineering/ez-tree风摆与RTC争抢project_vertex坑点.md)
- 坑点：[WebGPU下onBeforeCompile着色器机制失效坑点](../engineering/WebGPU下onBeforeCompile着色器机制失效坑点.md)
...
- 贴地设计文档：[ground-clamp.md](../../docs/design/ground-clamp.md)
