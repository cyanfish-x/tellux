...
> 1.0 施工中：本文描述施工前的内部装配与生命周期。公开入口（`viewer.layers`、`setTerrain()` 等）的目标形状以 `notes/架构/API稳定版破坏式变更方案.md` 批次 B 为准，不要按本文的公开路径继续实现。
...
flowchart TB
  ...
  Viewer --> PostProcessingManager["PostProcessingManager<br/>WebGL 后处理 pass 组合"]
  Viewer --> WebGPUPostProcessing["WebGPUPostProcessingManager<br/>TSL 后处理图"]
  ...
  PostProcessingManager --> Postprocessing["postprocessing<br/>EffectPass / NormalPass / BloomEffect"]
  WebGPUPostProcessing --> ThreeTSL["three/addons/tsl<br/>BloomNode / RenderPipeline"]
```
...
- `src/Clock.ts`：统一场景模拟时间源。负责当前时间、播放状态、倍率、逐帧推进与公开事件；Viewer 订阅时间变化并更新太阳、月亮和大气方向，其他业务消费者可通过相同事件接入。
...
- `src/rendering/PostProcessingManager.ts`：WebGL 后处理管理器。根据 `Scene` 状态组合 normal pass、大气云 pass、Bloom、lens flare、SMAA、dithering。
- `src/rendering/WebGPUPostProcessingManager.ts`：WebGPU 共享 TSL 后处理图。统一持有 scene pass、最终输出和有序 stage；Bloom、LensFlare、TAA 分别由独立 manager 注册并管理生命周期。
...
WebGPU renderer 由 `WebGPURendererAdapter` 统一开启 Three.js `highPrecision`，在 CPU 侧以 64 位精度预合并 model-view / normal-view 矩阵，避免普通 Mesh 和 NodeMaterial 在 ECEF 大数坐标下抖动。该开关不替代 `InstancedMesh`、`SkinnedMesh` 的 RTC / 局部原点方案，详细边界见 [WebGPU 地球大数坐标精度抖动坑点](../坑点记录/WebGPU地球大数坐标精度抖动坑点.md)。

构造阶段创建的可释放资源会立即登记到 `ResourceScope`。若后续任一同步装配步骤抛错，scope 会按创建顺序的逆序继续释放所有已登记资源；构造成功后则清空回滚栈。`Viewer.create()` 还会观察 renderer 的异步 `ready`，初始化拒绝时调用完整的 `destroy()`，因此 WebGPU 创建失败不会遗留 canvas、监听器或 manager。直接使用 `new Viewer()` 的调用方仍拥有失败清理责任。
