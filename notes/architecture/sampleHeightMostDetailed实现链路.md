...
> 状态：实现链路记录。2026-09-06 对照 `e849525` 的 `Viewer` / `HeightSampler` 修正公开契约与混合分流；其余算法细节保留原记录，本次没有重跑服务数据、浏览器或性能验证。

本文档记录 `Viewer.sampleHeightMostDetailed` 的当前实现逻辑，重点区分 terrain 直采、all 混合的采样专用 tileset，以及其他分流下的主场景局部预热。混合 all 路径不会把大量临时 region 挂到主场景 tileset 上。
...
- `src/rendering/ViewerRenderLoop.ts`
...
`HeightSampler.sampleHeightMostDetailedList` 先尝试混合路径：默认或 `source: 'all'`、同时存在 terrain 与可参与采样的场景 tileset 时，并行进行 terrain 直采和 `createHeightSamplingTilesets('tileset')` 采样专用 tileset 查询（离屏相机、`mask: true`）；两者都有高度时取较高值，仅一侧命中时保留该值。terrain 直采普通失败时混合路径保留 tileset 结果；取消仍以 `AbortError` 拒绝。
...
未进入 all 混合分支时，`source: 'tileset'` 或无 terrain 的 `source: 'all'` 可以通过 `createMostDetailedSamplingEntries` 使用 `TilesetManager.createSceneRegionHeightSamplingTilesets`。已有 terrain 且有场景 tileset 的 all 混合查询不会经过这一段。
...
`mask = false` 很重要：region 只会额外把采样区域纳入加载和细化，不会因 mask 剔除主相机视野内其他瓦片，但额外 traversal 和加载仍可能影响帧负载与缓存，不保证主画面完全不受影响。
...
采样专用 tileset 既用于 all 混合中的 3D Tiles 查询，也用于非混合路径的兜底：terrain 直采普通失败或主场景局部加载链路没有可用 tileset 时，尝试创建专用资源。
...
- `source: 'tileset'`：优先主场景 region preload；`source: 'all'` 同时有 terrain 与场景 tileset 时并行走 terrain 直采和采样专用 tileset。二者的 raycast 路径都依赖 LoadRegion，但缓存归属和对画面的影响不同。
