...
const flyToCityButton = document.querySelector<HTMLButtonElement>('#fly-to-city')
const attributionsElement = document.querySelector<HTMLElement>('#google-attributions')
...
tokenInput.value = ''
tokenInput.placeholder = DEFAULT_ION_TOKEN ? '留空使用 VITE_CESIUM_ION_TOKEN' : '输入 Cesium Ion token'
...
function summarizeTileMaterials(scene: THREE.Object3D) {
  ...
  scene.traverse((object) => {
    ...
    for (const material of materials) {
      ...
      materialTypes.set(materialType, (materialTypes.get(materialType) ?? 0) + 1)
...
function loadGooglePhotorealisticTiles() {
  ...
  if (!apiToken) {
    setStatus('请先输入 Cesium Ion token，或在 .env 中配置 VITE_CESIUM_ION_TOKEN。')
    return
  }
  ...
  ;(activeLayer.tileset as any).addEventListener('load-model', logTileMaterialDebug)
  flyToTokyo()
