...
export function stabilizeSplatRaycast(mesh: SplatMesh): void {
  ...
  mesh.raycast = function (raycaster, intersects) {
    ...
    // 分页数据仍走 Spark；视觉 LOD 的 raycastIndices 子集在推进时会漏检，改用完整高斯。
    // Paged data stays on Spark; visual-LOD raycastIndices miss while zooming in, so pick the full set.
    if (this.paged) return originalRaycast.call(this, raycaster, intersects)
